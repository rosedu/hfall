...
def drawterrain_once():
    ...
    for y in range(1, size):
        for x in range(1, size):
            verts.extend([x * vlen, y * vlen, vert[x][y],\
                          (x-1) * vlen, (y-1) * vlen, vert[x-1][y-1],\
                          x * vlen, (y-1) * vlen, vert[x][y-1],\
                          (x-1) * vlen, y * vlen, vert[x-1][y]])
            cols.extend([0, 0, color(x,y),\
                         0, 0, color(x-1,y-1),\
                         0, 0, color(x,y-1),\
                         0, 0, color(x-1,y)])
            indices.extend([4 * index, 4 * index + 1, 4 * index + 2,\
                            4 * index, 4 * index + 1, 4 * index + 3])
    verts = (GLfloat *len(verts))(*verts)
    cols = (GLfloat *len(cols))(*cols)
